...
int main(int argc, char **argv)
{
	...
	if (!strcmp(ovalue,"on")) /* set pins to on */
	{
		...
		{
			buf[0] = pins[0] | 0b10101010;
		}
		else if (!strcmp(pvalue, "1"))
		{
			buf[0] = pins[0] | 0b00000010;
		}
		else if (!strcmp(pvalue, "2")) 
		{
			buf[0] = pins[0] | 0b00001000;
		}
		else if (!strcmp(pvalue, "3"))
		{
			buf[0] = pins[0] | 0b00100000;
		}
		else if (!strcmp(pvalue, "4"))
		{
			buf[0] = pins[0] | 0b10000000;
		}
		else 
		{
			usage();
	...
	if (!strcmp(ovalue,"off")) /* set pins to off */
	{
		...
		{
			buf[0] = pins[0] & 0b11111101;
		}
		else if (!strcmp(pvalue, "2"))
		{
			buf[0] = pins[0] & 0b11110111;
		} 
		else if (!strcmp(pvalue, "3"))
		{
			buf[0] = pins[0] & 0b11011111;
		} 
		else if (!strcmp(pvalue, "4"))
		{
			buf[0] = pins[0] & 0b01111111;
		}
		else
		{
			usage();
	...
	int p1,p2,p3,p4;
	p1 = p2 = p3 = p4 = 0;
	p1 = pins[0] & 0b00000010; /* mask the pin */
	p1 = p1 >> 1;
	p2 = pins[0] & 0b00001000;
	p2 = p2 >> 3;
	p3 = pins[0] & 0b00100000;
	p3 = p3 >> 5;
	p4 = pins[0] & 0b10000000;
	p4 = p4 >> 7;

	printf("relay1:%i relay2:%i relay3:%i relay4:%i\n",p1,p2,p3,p4);
